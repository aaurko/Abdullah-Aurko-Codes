...
% u0 = 1+ 0.1*cos(pi*x0/3);
% v0 = zeros(size(x0));

%classical NLS soliton ICs
% mu = 1;
% u0 = sqrt(mu)*sech(sqrt(mu)*x0);
% v0 = zeros(size(x0));
...
for i=1:numel(tTest)
   ...
   HTest = UTest + 1i*VTest;
    ...
    % Calculate error.
%     err = norm(HNN - HTest) / norm(HTest);
err = norm(HNN - HTest, inf);

    % Plot predictions.
    subplot(1,4,i)
%     plot(extractdata(XTest),extractdata(sqrt(HPred(1,:).^2 + HPred(2,:).^2)),"-",LineWidth=2);
    plot(extractdata(XTest),extractdata(sqrt((HPred(1,:)-UTest).^2 + (HPred(2,:)-VTest).^2)),"-",LineWidth=2);
    ylim([0, 1.5])

    % Plot true values.
%     hold on
%     plot(extractdata(XTest),sqrt(UTest.^2 + VTest.^2), "--",LineWidth=2)
%     hold off
...
figure(3)
surface(T,X,abs(H_store-A*exp(2i*A^2*T))); shading interp
% surface(T,X,abs(H_store.'-A*exp(2*i*A^2*T))); shading interp
xlim([0 Ti]); ylim([-5 5])
axis tight
set(gca,'FontSize',18)
...
max_err=max(max(abs(H_store-A*exp(2i*A^2*T))))
